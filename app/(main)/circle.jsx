...
export default function CircleScreen() {
  console.log('Rendering CircleScreen — group details and member list');
  ...
  if (error) {
    return (
      <View style={[styles.center, { backgroundColor: c.canvas }]}>
        ...
        <TouchableOpacity onPress={fetchData}><Text style={[styles.retryText, { color: c.forest }]}>Retry</Text></TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.canvas }]}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={[styles.metaRow, { backgroundColor: c.surface }]}>
          <View style={styles.metaItem}>
            <Text style={[styles.metaLabel, { color: c.slateLight }]}>Goal</Text>
            <Text style={[styles.metaValue, { color: c.text }]}>{group?.goal_type || 'Land'}</Text>
          </View>
          <View style={styles.metaItem}>
            <Text style={[styles.metaLabel, { color: c.slateLight }]}>Tier</Text>
            <Text style={[styles.metaValue, { color: c.text }]}>{formatNaira(group?.contribution_tier || 0)}</Text>
          </View>
          <View style={styles.metaItem}>
            <Text style={[styles.metaLabel, { color: c.slateLight }]}>Members</Text>
...
const styles = StyleSheet.create({
  ...
  metaRow: { flexDirection: 'row', marginHorizontal: spacing.lg, borderRadius: 16, padding: spacing.md },
  metaItem: { flex: 1, alignItems: 'center', gap: 2 },
  metaLabel: { fontSize: fontSize.xs, fontFamily: 'Inter_400Regular', textTransform: 'uppercase' },
